#!/usr/bin/env python3
"""Bakes the two spark textures used by meshes/e/impact/*.nif.

    python3 tools/make_spark_textures.py

Both are additive sprites (the NIFs blend them SRC_ALPHA + ONE), so the alpha
channel is the brightness and the RGB is the tint. The tint runs white-hot in
the middle to blue at the edges - the "less yellow, a little bit blueish" look.
...
def make_point(size=32):
    """Round sparkle for the particle systems: hot core, quick falloff."""
    rows = []
    c = (size - 1) / 2.0
    for y in range(size):
        row = []
        for x in range(size):
            ...
            a = max(0.0, 1.0 - d) ** 2.2          # soft round falloff
            a = min(1.0, a * 1.35 + (0.9 if d < 0.16 else 0.0))  # blown out core
            row.append((*ramp(d * 0.95), a))
        rows.append(row)
...
def main():
    ...
    write_png(OUT / "spark_point.png", *make_point())
